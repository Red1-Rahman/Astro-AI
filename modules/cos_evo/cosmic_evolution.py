...
try:
    ...
    HAVE_21CMFAST = False
    logger.warning(
        "py21cmfast not available — using physically motivated analytic model."
    )
...
class CosmicEvolution:
    ...
    def _delta_T_b(self, z: float) -> float:
        ...
        cosmo_factor = np.sqrt((1.0 + z) / 10.0 * 0.15 / (Om0 * h**2)) * Ob_h2 / 0.023
    ...
    def _mock_brightness_temperature_cube(
        self, box_size: int, z: float, seed: int = 42
    ) -> np.ndarray:
        """
    ...
    def compute_power_spectrum_1d(
        self, cube: np.ndarray, kbins: int = 15, box_length: float = 50.0
    ):
    ...
    def _radial_average(self, arr, box_dims, kbins=10):
        ...
        outdata = np.histogram(k.flatten(), bins=kbins_arr, weights=arr.flatten())[0]
    ...
    def plot_global_evolution(self, save_path=None):
        ...
        ax.plot(
            self.results["redshifts"],
            self.results["global_signals"],
            "b-o",
            lw=2,
            ms=5,
            label="Global δTb",
    ...
    def plot_power_spectra_evolution(self, save_path=None):
        ...
        for i, z in enumerate(self.results["redshifts"]):
            ...
            if mask.sum() > 1:
                ax.loglog(
                    k[mask], ps[mask], color=colors[i], lw=2, label=f"z = {z:.1f}"
                )
    ...
    def plot_brightness_temperature_slices(self, z_indices=None, save_path=None):
        ...
        fig, axes = plt.subplots(1, len(z_indices), figsize=(5 * len(z_indices), 4))
        if len(z_indices) == 1:
            axes = [axes]
        ...
        if self.results.get("mock", False):
            fig.text(
                ...
                color="darkorange",
                bbox=dict(
                    boxstyle="round", fc="lightyellow", ec="darkorange", alpha=0.9
                ),
            )
    ...
    def generate_summary_report(self) -> dict:
        ...
        return {
            "data_provenance": {
                ...
                if self.results.get("mock")
                else "py21cmFAST simulation",
                "references": [
                    "Pritchard & Loeb 2012, Rev. Mod. Phys. 84, 1",
                    "Furlanetto et al. 2006, Phys. Rep. 433, 181",
                    "Mesinger et al. 2011, MNRAS 411, 955",
            ...
            "physical_interpretation": {
                "reionization_signature": bool(np.any(signals < -20)),
                "cosmic_dawn_absorption": bool(np.any(signals < -50)),
                "x_ray_heating_visible": bool(np.any(signals > 0)),
                "evolution_trend": "heating" if signals[-1] > signals[0] else "cooling",
            },
        }
    ...
    def export_results(self, output_path: str):
        ...
        if output_path.endswith(".npz"):
            ...
            for i, (k, ps) in enumerate(
                zip(self.results["power_spectra_k"], self.results["power_spectra_ps"])
            ):
