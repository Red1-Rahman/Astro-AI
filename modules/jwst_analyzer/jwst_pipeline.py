...
_REST_LINES = {
    ...
    0.49590: {
        "name": "[OIII]4959",
        "ratio": 0.340,
    },  # fixed ratio to 5007 (atomic physics)
...
def _apply_calzetti_attenuation(
    flux: np.ndarray, lam_um: np.ndarray, av: float
) -> np.ndarray:
    """
...
class JWSTAnalyzer:
    ...
    def _generate_mock_spectrum_1d(
        self,
        ...
        if not is_quenched:
            ...
            for lam_rest_line, props in line_ratios.items():
                ...
                profile = (
                    line_flux
                    / (sigma_lam * np.sqrt(2 * np.pi))
                    * np.exp(-((wavelength - lam_obs) ** 2) / (2 * sigma_lam**2))
                )
                flux += profile
    ...
    def extract_1d_spectrum(
        self, extraction_method: str = "optimal", source_id: str = "target"
    ) -> dict | None:
        if not self.pipeline_status["stage2"]:
        ...
        wavelength_1d = (
            wl_2d[data_2d.shape[0] // 2, :]
            if wl_2d is not None
            else np.linspace(_NIRSPEC_LAMBDA_MIN, _NIRSPEC_LAMBDA_MAX, data_2d.shape[1])
        )
        ...
        spec = methods.get(extraction_method, self._optimal_extraction)(
            data_2d, error_2d, wavelength_1d
        )
    ...
    def _aperture_extraction(self, data_2d, error_2d, wavelength) -> dict:
        ...
        err_1d = (
            np.sqrt(np.sum(error_2d[y_lo:y_hi, :] ** 2, axis=0))
    ...
    def setup_spectral_fitting_model(
        self, sfh_model: str = "exponential", include_nebular: bool = True
    ) -> dict:
    ...
    def fit_spectrum_bagpipes(
        ...
        try:
            ...
            spec_arr = np.column_stack([wl, spectrum["flux"], spectrum["flux_error"]])
            galaxy = pipes.galaxy(
                source_id, lambda gid: spec_arr, spectrum_exists=True, spec_wavs=wl
            )
            ...
            result = {
                ...
                "chi2": post.samples.get("chisq_phot", np.array([np.nan])).mean(),
                "mock": False,
            }
    ...
    def _mock_spectral_fitting(self, spectrum: dict, source_id: str) -> dict:
        ...
        age_rec = float(
            np.clip(rng.normal(t_universe * 0.5, t_universe * 0.2), 0.1, t_universe)
        )
    ...
    def plot_1d_spectrum(
        self, source_id: str = "target", show_fit: bool = True, save_path=None
    ):
        if source_id not in self.extracted_spectra:
            raise ValueError(f"No extracted spectrum for '{source_id}'.")
        ...
        axes[0].fill_between(
            wl, flux - err, flux + err, alpha=0.25, color="gray", label="±1σ"
        )
        ...
        if show_fit and source_id in self.spectral_fits:
            ...
            if "model_spectrum" in fit:
                ...
                axes[0].plot(mwl, mflx, "r-", lw=1.2, alpha=0.8, label="Best-fit model")
            ...
            for lam_rest, props in _REST_LINES.items():
                ...
                if _NIRSPEC_LAMBDA_MIN < lam_obs < _NIRSPEC_LAMBDA_MAX:
                    axes[0].axvline(
                        lam_obs, color="steelblue", lw=0.6, ls=":", alpha=0.7
                    )
                    axes[0].text(
                        lam_obs,
    ...
    def plot_spectral_fit_results(self, source_id: str = "target", save_path=None):
        ...
        if source_id in self.extracted_spectra:
            spec = self.extracted_spectra[source_id]
            axes[1, 0].plot(
                spec["wavelength"], spec["flux"], "k-", lw=0.8, label="Observed"
            )
            if "model_spectrum" in fit:
                m = fit["model_spectrum"]
                axes[1, 0].plot(m["wavelength"], m["flux"], "r-", lw=1, label="Model")
    ...
    def export_results(self, output_path: str, format: str = "fits"):
        if format.lower() == "fits" and HAVE_ASTROPY:
            ...
            for sid, spec in self.extracted_spectra.items():
                tbl = Table(
                    [spec["wavelength"], spec["flux"], spec["flux_error"]],
