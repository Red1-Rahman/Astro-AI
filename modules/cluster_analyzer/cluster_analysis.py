...
def _double_schechter_logm(log_m: np.ndarray, params: dict) -> np.ndarray:
    ...
    term1 = (
        params["phi1"]
        * np.log(10)
        * 10 ** (dm * (1 + params["alpha1"]))
        * np.exp(-(10**dm))
    )
    term2 = (
        params["phi2"]
        * np.log(10)
        * 10 ** (dm * (1 + params["alpha2"]))
        * np.exp(-(10**dm))
    )
    return term1 + term2
...
def _speagle_sfr(
    log_m: float, t_gyr: float = _SFMS_T_REF, scatter_dex: float = 0.3
) -> float:
    """
    ...
    """
    log_sfr_mean = (0.84 - 0.026 * t_gyr) * log_m - (6.51 - 0.11 * t_gyr)
...
def _sample_log_mass(
    n: int, params: dict, log_m_min: float = 9.0, log_m_max: float = 12.0
) -> np.ndarray:
    """
...
class ClusterAnalyzer:
    ...
    def generate_mock_data(
        self, n_galaxies: int = 1000, n_clusters: int = 5, z_cluster: float = 1.0
    ) -> bool:
        """
        ...
        # --- cluster members ---
        log_m_cluster = _sample_log_mass(
            n_cluster_gals, _SCHECHTER_CLUSTER, log_m_min=9.5
        )
        for i in range(n_cluster_gals):
            ...
            sfr = (
                _speagle_sfr(log_m, t_gyr, scatter_dex=0.25) * 0.3
                if is_red
        ...
        # --- field galaxies ---
        log_m_field = _sample_log_mass(n_field_gals, _SCHECHTER_FIELD, log_m_min=8.5)
        for i in range(n_field_gals):
            log_m = log_m_field[i]
            f_red = _peng_red_fraction(log_m, in_cluster=False)
            is_red = rng.random() < f_red
            sfr = (
                _speagle_sfr(log_m, t_gyr, scatter_dex=0.3) * 0.1
                if is_red
        ...
        logger.info(
            "Generated %d mock galaxies (Baldry+2012 SMF, Peng+2010 f_red, "
            "Speagle+2014 SFMS).",
            len(self.data),
        )
        return True
    ...
    def detect_clusters_redshift(
        self, z_bins: int = 50, significance_threshold: float = 3.0
    ) -> dict:
        if self.data is None:
            raise ValueError("No data loaded.")
        ...
        peaks = [
            ...
            if (
                significance[i] > significance_threshold
    ...
    def setup_bagpipes_model(
        self, sfh_model: str = "exponential", dust_model: str = "Calzetti"
    ) -> dict:
    ...
    def run_sed_fitting(
        self, galaxy_subset=None, fit_instructions=None, n_live: int = 400
    ) -> dict:
    ...
    def _mock_sed_fitting_results(self, galaxy_subset: pd.DataFrame) -> dict:
        ...
        for _, galaxy in galaxy_subset.iterrows():
            ...
            if is_red:
                ...
                log_sfr = (
                    _speagle_sfr.__wrapped__(log_m, t_gyr)
                    if hasattr(_speagle_sfr, "__wrapped__")
                    else ((0.84 - 0.026 * t_gyr) * log_m - (6.51 - 0.11 * t_gyr) - 2.0)
                )
            ...
            av = rng.exponential(0.4) if not is_red else rng.exponential(0.1)
    ...
    def compute_red_fraction(self, mass_bins=None) -> dict:
        ...
        def _rf(gals, bins):
            ...
            for i in range(len(bins) - 1):
                mask = (
                    (gals["log10M"] >= bins[i]) & (gals["log10M"] < bins[i + 1])
                    if "log10M" in gals.columns
                    else np.zeros(len(gals), bool)
                )
                sub = gals[mask]
                n = len(sub)
    ...
    def plot_color_magnitude_diagram(self, save_path=None):
        ...
        for gals, color, label in [
            ...
            if "r_mag" in gals.columns and "g_r" in gals.columns:
                ax.scatter(
                    gals["r_mag"], gals["g_r"], alpha=0.5, s=15, c=color, label=label
                )
    ...
    def plot_stellar_mass_functions(self, save_path=None):
        ...
        ax1.semilogy(
            mc,
            res["reference"]["baldry2012_field"],
            "b--",
            alpha=0.5,
            label="Baldry+2012 (field)",
        )
        ax1.semilogy(
            mc,
            res["reference"]["vulcani2013_cluster"],
            "r--",
            alpha=0.5,
            label="Vulcani+2013 (cluster)",
        )
        ...
        for label, phi, ls, color in [
            ...
            mask = phi > 0
            if mask.sum() > 0:
                ax2.semilogy(mc[mask], phi[mask], ls, color=color, label=label, lw=2)

        ax2.set_xlabel("log(M*/M☉)")
        ax2.set_ylabel("φ [Mpc⁻³ dex⁻¹]")
        ax2.set_title("SMF by Color")
    ...
    def plot_red_fraction(self, save_path=None):
        ...
        ax1.plot(
            mc,
            res["reference"]["peng2010_cluster"],
            "r--",
            alpha=0.6,
            label="Peng+2010 (cluster)",
        )
        ax1.plot(
            mc,
            res["reference"]["peng2010_field"],
            "b--",
            alpha=0.6,
            label="Peng+2010 (field)",
        )
        ...
        ax2.errorbar(
            ...
            ms=5,
            capsize=3,
            label="Δ f_red (measured)",
        )
        ax2.axhline(
            _PENG_ENV_DELTA_RED_FRAC,
            color="green",
            ls="--",
            alpha=0.6,
        ...
        ax2.set_xlabel("log(M*/M☉)")
        ax2.set_ylabel("Δ f_red = cluster − field")
    ...
    def generate_summary_report(self) -> dict:
        ...
        if "is_red" in self.data.columns:
            report["color_statistics"] = {
                "cluster_red_fraction": float(cl_gals["is_red"].mean())
                if len(cl_gals) > 0
                else 0,
                "field_red_fraction": float(fi_gals["is_red"].mean())
                if len(fi_gals) > 0
                else 0,
                "environmental_excess": float(
                    cl_gals["is_red"].mean() - fi_gals["is_red"].mean()
                )
                if len(cl_gals) > 0 and len(fi_gals) > 0
                else 0,
        ...
        if "log10M" in self.data.columns:
            report["mass_statistics"] = {
                "cluster_median_log_mass": float(cl_gals["log10M"].median())
                if len(cl_gals) > 0
                else 0,
                "field_median_log_mass": float(fi_gals["log10M"].median())
                if len(fi_gals) > 0
                else 0,
                "log_mass_range": [
                    float(self.data["log10M"].min()),
...
def _mock_photometry(
    log_m: float, is_red: bool, rng: np.random.Generator
) -> tuple[float, float]:
    """
