...
class Dashboard:
    ...
    def _generate_mock_spectrum_for_plot(
        ...
        *redshift* before plotting, so they fall in the correct observed
        ...
        continuum = np.maximum(1e-18 * (lam_rest**-1.5) * np.exp(-lam_rest / 1.5), 0.0)
        # Lyman break
    ...
    def create_timeline_integration(self, save_path=None):
        ...
        if self.cos_evo_results:
            z_21cm = np.asarray(
                self.cos_evo_results.get("redshifts", np.linspace(6, 15, 10))
            )
            sig_21cm = np.asarray(
                self.cos_evo_results.get("global_signals", np.zeros_like(z_21cm))
            )
            t_21cm = self._redshift_to_time(z_21cm)
            ax1.plot(t_21cm, sig_21cm, "b-", lw=3, label="21cm Global Signal")
            ax1.fill_between(
                t_21cm, sig_21cm - 5, sig_21cm + 5, alpha=0.25, color="blue"
            )
        ...
        if (
            ...
            ax2.semilogx(
                10**mc, rf["field"]["red_fraction"], "bo-", lw=2, ms=6, label="Field"
            )
        else:
        ...
        if (
            ...
            if jm:
                ax5.scatter(
                    jm, js, c="blue", s=80, alpha=0.8, marker="s", label="JWST spectra"
                )
        else:
        ...
        fig.suptitle(
            "Astro-AI: Integrated Galaxy Evolution Analysis",
            fontsize=15,
            fontweight="bold",
            y=0.97,
        )
    ...
    def create_environment_comparison(self, save_path=None):
        ...
        fig.suptitle(
            "Environmental Effects on Galaxy Properties", fontsize=16, fontweight="bold"
        )
        ...
        if (
            ...
            if isinstance(cg, pd.DataFrame) and "ra" in cg.columns:
                ax.scatter(
                    fg["ra"], fg["dec"], alpha=0.6, s=15, c="steelblue", label="Field"
                )
                ax.scatter(
                    cg["ra"], cg["dec"], alpha=0.8, s=30, c="tomato", label="Cluster"
                )
        else:
        ...
        ax.plot(
            r_rng, 0.65 + 0.02 * (r_rng - 22), "k--", alpha=0.7, label="Red sequence"
        )
        ax.set_xlabel("r [mag]")
        ax.set_ylabel("g – r")
        ...
        z_cl = np.concatenate([rng.normal(1.2, 0.05, 150), rng.normal(1.8, 0.03, 50)])
        ax.hist(
            z_field,
            bins=z_bins,
            alpha=0.6,
        ...
        ax.hist(
            z_cl, bins=z_bins, alpha=0.8, color="tomato", label="Cluster", density=True
        )
        ax.axvline(1.2, color="tomato", ls="--", alpha=0.7, label="Cluster z")
        ax.set_xlabel("Redshift")
        ax.set_ylabel("Normalised Number")
    ...
    def create_jwst_showcase(self, save_path=None):
        ...
        fig.suptitle(
            "JWST Spectroscopic Analysis Results", fontsize=16, fontweight="bold"
        )
        ...
        ax5.errorbar(
            ...
            ms=8,
            capsize=5,
            label="JWST (this work)",
        )
        ax5.plot(x + 0.1, vals_lit, "bs", ms=8, label="Literature median")
        ax5.set_xticks(x)
        ax5.set_xticklabels(props, rotation=15, ha="right")
    ...
    def create_summary_dashboard(self, save_path=None):
        ...
        if self.cos_evo_results:
            ...
            s_v = np.asarray(
                self.cos_evo_results.get("global_signals", np.zeros_like(z_v))
            )
        else:
        ...
        ax_cm.set_xlabel("Redshift")
        ax_cm.set_ylabel("δTb [mK]")
        ax_cm.set_title("Cosmic Evolution", fontweight="bold")
        ...
        ax_sp.set_xlabel("Wavelength [μm]")
        ax_sp.set_ylabel("Flux")
        ax_sp.set_title("JWST Spectroscopy", fontweight="bold")
        ...
        ax_ms.scatter(
            ms_m[~cl_m], ms_s[~cl_m], c="steelblue", s=20, alpha=0.6, label="Field"
        )
        ax_ms.scatter(
            ms_m[cl_m], ms_s[cl_m], c="tomato", s=20, alpha=0.8, label="Cluster"
        )
        ax_ms.plot(
            np.linspace(9.5, 11.5, 50),
            np.linspace(9.5, 11.5, 50) - 9.0,
            "k--",
            alpha=0.6,
            label="SFMS",
        )
        ax_ms.set_xlabel("log(M*/M☉)")
        ax_ms.set_ylabel("log(SFR)")
        ax_ms.set_title("Mass–SFR", fontweight="bold")
        ax_ms.legend(fontsize=8)
        ax_ms.grid(alpha=0.3)
        ...
        for z_ep, label in [
            ...
            ax_tl.text(
                t_ep, 0.95, label, ha="center", va="bottom", fontsize=8, rotation=0
            )

        ax_tl.set_xlim(0.3, 13.8)
        ax_tl.set_ylim(0, 1.05)
    ...
    def generate_integration_report(self) -> dict:
        ...
        if self.cos_evo_results:
            ...
            report["cosmic_evolution_summary"] = {
                ...
                "signal_range_mK": [float(min(gs)), float(max(gs))]
                if len(gs)
                else None,
